#! python
# -*- coding: utf-8 -*-
# $Id: zh-Hans.py 146 2006-11-07 07:44:29Z sgrayban $
...
def AddStrings(strtable):
    ...
    add("str_goto_background_on_close_warn", \
        u"当关闭主窗口时，CastPodder可以在后台运行或退出，\n" \
        u"你希望CastPodder继续运行？")
    ...
    add("str_set_catchup_description", \
        u"当使用跟进动作时，CastPodder只是处理每个Feed中最新的一个Podcast\n" \
        u"请选择CastPodder应该怎样处理跳过的条目")
    ...
    add("str_set_oneclick_handler_warn",\
        u"CastPodder不是你当前的Podcast单击订阅处理器。\n" \
        u"当点击单击订阅链接时，启动CastPodder吗？")
    add("str_ensure_oneclick_handler", u"总是使用CastPodder进行单击订阅")
    ...
    add("str_gen_options_expl", u"为CastPodder程序设置通用选项")
    add("str_hide_on_startup", u"启动后只在系统托盘中显示CastPodder")
    ...
    add("str_initialized", u"CastPodder 准备完成")
    ...
    import gui.skin
    add("str_ipodder_title", gui.skin.PRODUCT_NAME + u" - Podcast 接收机 v" + __version__)
    add("str_localization_restart", u"CastPodder的语言切换需要重新启动，点击确定重新启动，点击取消继续")
    add("str_really_quit", u"一个下载进程在运行，真的退出？");
    add("str_double_check", u"已经有一个下载进程在运行");
    
    # check for update
    add("str_new_version_ipodder", u"已经有新版本的CastPodder了，去下载页面")
    add("str_no_new_version_ipodder", u"这个版本的CastPodder已经是最新的")
    add("str_other_copy_running", u"另一个CastPodder正在运行中，你可以选择打开它或等它完成或中止它")

    # Windows taskbar right-click menu
    add("str_check_now", u"立刻检查")        
    add("str_open_ipodder", u"打开CastPodder")
    #--- Downloading
