#! python
# -*- coding: utf-8 -*-
# $Id: it.py 146 2006-11-07 07:44:29Z sgrayban $
...
def AddStrings(strtable):
    ...
    add("str_goto_background_on_close_warn", \
        u"CastPodder può continuare a lavorare in background dopo la chiusura \n" \
        u"della finestra principale. In alternativa CastPodder può essere chiuso. \n" \
        u"Desideri mantenere attivo CastPodder?")
    ...
    add("str_set_catchup_description", \
        u"When checking in Catchup mode, CastPodder will skip all but the top \n" \
        u"item in each feed. Please specify how CastPodder should treat the \n" \
        u"skipped items.")
    ...
    add("str_set_oneclick_handler_warn",\
        u"CastPodder is not currently your one-click subscription handler for podcasts. \n" \
        u"Should we set CastPodder to launch from one-click subscription links?")
    ...
    add("str_donate", u"Fai una donazione a CastPodder")
    add("str_donate_expl", u"E' importante mantenere in vita la comunità di CastPodder e continuare a fornire questo nuovo modo di consumare file multimediali libero. Qualsiasi somma di denaro è ben gradita dal team di sviluppo ed incoraggerà la realizzazione di nuove funzioni e servizi!")
    ...
    add("str_gen_options_expl", u"Imposta le opzioni generali per il programma CastPodder")
    add("str_hide_on_startup", u"Mostra CastPodder nella system tray all'avvio")
    ...
    add("str_initialized", u"CastPodder pronto")
    ...
    # check for update
    add("str_new_version_ipodder", u"E' disponibile una nuova versione di CastPodder, premi Ok per aprire la pagina di download.")
    add("str_no_new_version_ipodder", u"La versione di CastPodder in uso è la più recente")
    add("str_other_copy_running", u"Un'altra istanza di CastPodder è in esecuzione. Attendere il completamento o chiudere l'istanza.")

    # Windows taskbar right-click menu
    add("str_check_now", u"Controlla Ora")        
    add("str_open_ipodder", u"Apri CastPodder")
    #--- Downloading
