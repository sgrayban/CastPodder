#! python
# -*- coding: utf-8 -*-
# $Id: de.py 146 2006-11-07 07:44:29Z sgrayban $
...
def AddStrings(strtable):
    ...
    add("str_delete_downloads", u"Alle Downloads löschen")
    add("str_del_wintitle", u"Downloads löschen")
    add("str_del_ask", u"Sind Sie sicher, daß Sie ")
    add("str_del_ask1", u" Files und ")
    add("str_del_ask2", u" Megabytes im Download Verzeichnis löschen wollen?")
    add("str_sync_mediaplayer", u"Mediaplayer synchronisieren")

    add("str_donate_systray", u"Spenden")
    add("str_copy_location", u"Zielverzeichnis")
    add("str_username", u"Username")
    add("str_password", u"Passwort")
    add("str_missing_proxy_password", u"Es gibt einen Proxy Usernamen, aber kein Proxy Passwort.\nBitte entweder den Usernamen löschen, oder ein Passwort eingeben.")

    add("str_goto_background_on_close_title", u"Schliess Verhalten einstellen")
    add("str_goto_background_on_close_warn", u"CastPodder kann weiter im Hintergrund laufen, nachdem das Hauptfenster\n geschlossen wurde, oder CastPodder kann beendet werden.  Möchten Sie, dass CastPodder weiter\nläuft?")
    add("str_goto_background_on_close_pref", u"Weiter in Hintergrund laufen, wenn das Hauptfenster geschlossen wird?")
    add("str_yes", u"Ja")
    add("str_no", u"Nein")
    add("str_dont_ask", u"Bitte nicht noch einmal fragen")
    add("str_ok", u"OK")
    add("str_hide_window", u"Fenster ausblenden")
    add("str_hide_tray_icon", u"Tray Icon ausblenden")
    add("str_show_window", u"Fenster zeigen")

    add("str_catchup_pref", u"Catchup überspringt ältere Episoden dauerhaft")
    add("str_set_catchup_title", u"Catchup Verhalten einstellen")
    add("str_set_catchup_description", u"Im Catchup Modus überspringt CastPodder alle bis auf die Top\nEinträge in jedem Feed.  Was soll CastPodder mit den übersprungenen\n Folgen machen?")
    add("str_skip_permanently", u"Dauerhaft überspringen")
    add("str_skip_temporarily", u"Nur dieses Mal überspringen")
    
    add("str_set_oneclick_handler", u"One-Click Handler einstellen")
    add("str_set_oneclick_handler_warn", u"CastPodder ist momentan nicht Ihr One-Click Abonnement Handler für Podcasts.\nSoll CastPodder als One-Click Abonnenment Handler fungieren?")
    add("str_ensure_oneclick_handler", u"CastPodder immer als One-Click Abonnement nutzen")
    
    add("str_txt_feedmanager", u"Kompatible Feedmanager:")
    add("str_feedmanager_btn_podnova", u"www.PodNova.com - Podcasts durchsuchen, One-Click Abonnement")
    add("str_feedmanager_btn_gigadial", u"www.gigadial.net - Podcasts durchsuchen, One-Click Abonnement")

    add("str_open_downloads_folder", u"Download Verzeichnis öffnen")
    add("str_chkupdate_on_startup", u"Neue CastPodder Version bei Systemstart überprüfen.")
    add("str_bad_feedmanager_url", u"Bitte eine gültige URL für den Feed Manager eingeben.")
    add("str_feed_manager", u"Feed Manager")
    add("str_feedmanager_enable", u"Meine Abonnements von einem Remote Service synchronisieren")
    add("str_opml_url", u"OPML URL")
    add("str_set_track_genre", u"Genre einstellen")
    add("str_auto_delete", u"Automatisch Episoden löschen, die mehr als")
    add("str_days_old", u"Tage alt sind")
    
    add("str_show_notes", u"Aufzeichnung anzeigen")
    add("str_close", u"Schliessen")
    
    add("str_critical_error_minspace_exceeded", u"Download übersprungen; Freier Speicher %dMB weniger als Minimum %dMB.  Bitte Speicher auf der Festplatte freigeben, oder die Speicher Management Einstellungen verändern")
    add("str_critical_error_unknown", u"Unbekannter kritischer Fehler während des Downloads.")
    ...
    add("str_donate", u"Für CastPodder spenden")
    add("str_donate_expl", u"Es ist wichtig, diese der Allgemeinheit gehörenden, auf CastPodder basierenden Programme frei für alle zur Verfügung stellen zu können, denn jeder soll die Möglichkeit haben, auf diese neue Art Audio, Video und mehr nutzen zu können. Jeder Betrag macht das Team glücklich und motiviert die Teilnehmer, für Sie an neuen Features und Diensten zu arbeiten!")
    ...
    add("str_ep_to_download", u"Noch herunterzuladen")
    ...
    add("str_make_donation", u"Ipodder unterstützen")
    ...
    add("str_gen_options_expl", u"Generelle Einstellungen für CastPodder")
    ...
    add("str_initialized", u"CastPodder ist bereit")


    ## Other application strings
    from ipodder.configuration import __version__
    add("str_ipodder_title", u"CastPodder - Podcast-Lader v" + __version__)
    add("str_localization_restart", u"Um alle Einstellungen zu lokalisieren, ist ein Neustart von CastPodder notwendig. Klicken Sie OK um CastPodder zu beenden, Abbrechen um weiterzuarbeiten.")
    ...
    # check for update
    add("str_new_version_ipodder", u"Es ist eine neue Version von CastPodder verfügbar, drücken Sie OK, um auf die Webseite zu wechseln.")
    add("str_no_new_version_ipodder", u"Ihre CastPodder-Version ist die aktuelleste!")
    add("str_other_copy_running", u"Eine andere Instanz von CastPodder ist bereits gestartet. Bitte verwenden Sie diese, warten Sie darauf daß es erscheint oder beenden Sie den Prozeß.")

    # Windows taskbar right-click menu
    add("str_check_now", u"Jetzt überprüfen")
    add("str_open_ipodder", u"CastPodder öffnen")
    #--- Downloading
