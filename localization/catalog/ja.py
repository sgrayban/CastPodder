#! python
# -*- coding: utf-8 -*-
# $Id: ja.py 146 2006-11-07 07:44:29Z sgrayban $
...
def AddStrings(strtable):
    ...
    add("str_about", u"CastPodder について")
    ...
    add("str_gen_options_expl", u"CastPodder の全般的なオプションを設定")
    add("str_hide_on_startup", u"スタートアップ時に CastPodder をシステムトレイに置く")
    ...
    add("str_initialized", u"CastPodder は準備ができました")
    ...
    add("str_ipodder_title", u"CastPodder - Podcast receiver v" + __version__)
    add("str_localization_restart", u"CastPodder の言語設定の変更には再起動が必要です。CastPodder を終了してもよいときは、Ok をクリックしてください。続けるには Cancel をクリックしてください。")
    add("str_really_quit", u"ダウンロード中です。本当に CastPodder を終了しますか？");
    add("str_double_check", u"ダウンロードが始まっているようです。終了しますか？");
    
    # check for update
    add("str_new_version_ipodder", u"CastPodder の新しいバージョンが利用可能です。ダウンロードサイトに行くには Ok をクリックしてください。")
    add("str_no_new_version_ipodder", u"今お使いの CastPodder は最新です。")
    add("str_other_copy_running", u"CastPodder が別に実行中です。そちらを終了もしくは強制終了させてください")

    # Windows taskbar right-click menu
    add("str_check_now", u"今すぐ新着の確認をする")
    add("str_open_ipodder", u"CastPodder を開く")
