#! python
# -*- coding: utf-8 -*-
# $Id: ru.py 146 2006-11-07 07:44:29Z sgrayban $
...
def AddStrings(strtable):
    ...
    add("str_donate", u"Внести пожертвование CastPodder")
    add("str_donate_expl", u"Очень важно сохранить ПО CastPodder-сообщества и поддерживать его дальнейшее развитие. Небольшое денежное пожертвование сможет сделать разработчиков счастливыми и дать стимул для дальнейшей разработки и введению новых сервисов!")
    ...
    add("str_ep_to_download", u"Закачать")
    ...
    add("str_gen_options_expl", u"Установите главные настройки CastPodder'а")
    add("str_hide_on_startup", u"Прятать CastPodder в трей при старте")
    ...
    add("str_initialized", u"CastPodder готов")
    ...
    import gui.skin
    add("str_ipodder_title", gui.skin.PRODUCT_NAME + u" - Подкаст-менеджер v" + __version__)
    ...
    # check for update
    add("str_new_version_ipodder", u"Доступна новая версия CastPodder, нажмите Ok для перехода на сайт.")
    add("str_no_new_version_ipodder", u"Эта версия CastPodder - последняя")
    add("str_other_copy_running", u"Другая копия CastPodder уже запущена. Пожалуйста, дождитесь её закрытия или прервите её выполнение.")

    # Windows taskbar right-click menu
    add("str_check_now", u"Проверить сейчас")        
    add("str_open_ipodder", u"Открыть CastPodder")
    #--- Downloading
