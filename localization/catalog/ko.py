#! python
# -*- coding: utf-8 -*-
# $Id: ko.py 146 2006-11-07 07:44:29Z sgrayban $
...
def AddStrings(strtable):
    ...
    add("str_goto_background_on_close_warn", \
        u"CastPodder는 창을 닫은 후에도 백그라운드에서 실행이 가능합니다. \n" \
        u" 백그라운드 실행을 사용하시겠습니까?")
    ...
    add("str_set_oneclick_handler_warn",\
        u"CastPodder는 현재 원클릭구독신청을 지원하지 않습니다. \n" \
        u"CastPodder에서 원클릭구독신청을 사용하게 할까요?")
    ...
    add("str_donate", u"CastPodder에 기부하기")
    add("str_donate_expl", u"기부는 커뮤니티기반의 CastPodder 프로그램을 계속 진행하고, 무료 미디어의 소비를 유지하는 데 중요한 일입니다. 크거나 작은 모든 도움들이 개발팀을 기쁘게하고 그들이 원하는 새로운 기능이나 서비스의 개발에 도움이 될것입니다!")
    ...
    add("str_ep_to_download", u"다운로드예정")
    ...
    add("str_initialized", u"CastPodder 준비완료")
    ...
    # check for update
    add("str_new_version_ipodder", u"CastPodder 새 버전이 나왔습니다. 확인을 선택하시면 다운로드 사이트로 이동합니다.")
    add("str_no_new_version_ipodder", u"업데이트버전이 없습니다.")
    add("str_other_copy_running", u"다른버전의 CastPodder가 작동중입니다. 작업이 끝나기를 기다리거나, 종료시켜주십시오.")

    # Windows taskbar right-click menu
    add("str_check_now", u"새 방송 확인")        
    add("str_open_ipodder", u"CastPodder 열기")
    #--- Downloading
