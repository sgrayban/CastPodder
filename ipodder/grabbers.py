#
# CastPodder URL grabbing
# Copyright (c) 2005-2006 Scott Grayban and the CastPodder Team
#
# CastPodder file grabbing code
#
# $Id: grabbers.py 147 2006-11-07 08:17:03Z sgrayban $
...
### fix from EJ Finneran
### Does not work yet - sgrayban
...
class BasicGrabber(GenericGrabber): 
    ...
    def _open(self): 
        ...
        # If we want to use proxy servers we need 
        # to prepare a handler to do so. 
        handlers = []
        ...
        #if proxies: 
        #    # BROKEN PROXY CODE WILl ROBINSON.
        #    self.spam("Using proxies: %s", 
        #             ", ".join(
        ...
        #    handlers.append(urllib2.ProxyHandler(proxies))
            ...
            if self.http_proxy_username and self.http_proxy_password:
                # AG: urllib2 is a little quirky, at least in Python 2.3.
                # ProxyBasicAuthHandler matches the password against the proxy
                # host, which should not contain the string "http://".
                # ProxyDigestAuthHandler matches the password
                # against the target URI, which *should* contain "http://".
                # Finally, ProxyDigestAuthHandler should be tried last, because
                # it blows out with an exception if the auth method doesn't
                # match.  Happily, ProxyBasicAuthHandler fails quietly so
                # we can try it first.
                passman = urllib2.HTTPPasswordMgrWithDefaultRealm()
                passman.add_password(None, host, self.http_proxy_username, self.http_proxy_password)
        ...
        try: 
            #MAXLOOP = 10    # sounds reasonable enough
            #loopcount = 0   # Keep track of the number of redirects
            #while 1:
               ...
               headers = fp.info()
               #if (hasattr(fp, 'code') and not (fp.code == 301 or fp.code == 302 or fp.code == 303 or fp.code == 307)):
               #   break
               #else:
               #   self.debug("Got %s response; new location sent %s (redirect from %s)", fp.code, repr(headers.get('location')), fp.geturl())
               #   self.what = headers.get('location')
               #   loopcount = loopcount + 1
               #   if loopcount > MAXLOOP:
               #      raise GrabError, ("Number of redirects greater than max allowed (%d)" % MAXLOOP)
               #      break
               #   self.spam("Calling _open_resource again... got a redirect")
        ...
        if hasattr(fp, 'code'): 
            self.debug('code: %s', repr(fp.code))
            ...
            elif fp.code == 301: # remember the new location
                self.debug("Got 301 response; didn't remember new location "\
                         "%s (was %s)", fp.geturl(), self.what)

            elif fp.code == 416: # can't satisfy range
